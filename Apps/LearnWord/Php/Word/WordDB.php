...
class WordDB  extends SqlDBUtil
{
	...
	const KEY_date = 'date'; //含义
	const KEY_addtime = 'addtime'; //含义

	public $arrayCol = array(
		self::KEY_id, self::KEY_word, self::KEY_pinyin, self::KEY_zuci,
		self::KEY_bushou, self::KEY_bihua, self::KEY_audio, self::KEY_gif, self::KEY_mean, self::KEY_date, self::KEY_addtime
	);
	public $arrayColType;
	...
	public function ReadInfo($item)
	{
		...
		$info->date =   $item[self::KEY_date];
		$info->addtime =   $item[self::KEY_addtime];
		return $info;
	}
}
